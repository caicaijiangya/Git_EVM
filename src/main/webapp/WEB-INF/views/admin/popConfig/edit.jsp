...
$(function() {
	var image_1 = '${popConfig.image1}';
	var image_2 = '${popConfig.image2}';
	if(image_1 != null && image_1.length > 0){
		image1 = image_1;
		var imgUrl = '"'+image_1+'"';
		imgHtml = "<a href='"+image_1+"' target='_blank'><img src='"+image_1+"' width='120' height='120'/></a><input type='button' onclick='delImg(this,1)' value='删除' />";
		$("#image1Desc").append(imgHtml);
	}
	if(image_2 != null && image_2.length > 0){
		image2 = image_2;
		imgHtml = "<a href='"+image_2+"' target='_blank'><img src='"+image_2+"' width='120' height='120'/></a><input type='button' onclick='delImg(this,2)' value='删除' />";
	...
	$('#popConfigEditForm').form({
        url : '${path }/popConfig/edit',
        onSubmit : function() {
            progressLoad();
...
<div class="easyui-layout" data-options="fit:true,border:false" >
    <div data-options="region:'center',border:false" style="overflow-y:scroll; padding: 3px;" >
        <form id="popConfigEditForm" method="post" enctype="multipart/form-data">
        	<input type="hidden" name="id" value="${popConfig.id }" />
            ...
            <table class="grid">
                ...
                <tr>
                    ...
                    <td><input name="title" type="text" value="${popConfig.title }" placeholder="请输入标题" class="easyui-validatebox span2" data-options="required:true"></td>
                </tr>
                <tr>
                	<td align="right" bgcolor="#f8f8f8">类型</td>
                    <td>
                    	<input name="type" value="0" onclick="selectCoupon(0)" type="radio" ${popConfig.type==0?'checked':''}>普通
                    	<input name="type" value="1" type="radio" onclick="selectCoupon(1)" ${popConfig.type==1?'checked':''}>优惠券领取
                    </td>
                </tr>
                <tr id="coupon" style="${popConfig.type==0?'display: none':''}">
                    ...
                    <td>
	                    <select id="couponId" name="couponId" style="width: 200px; height: 29px;" class="easyui-combobox">
	                    	<c:forEach items="${couponList }" var="item">
	                    		<option value="${item.id }" <c:if test="${popConfig.couponId == item.id }">selected="selected"</c:if>>${item.goodsName }</option>
	                    	</c:forEach>
                ...
                <tr>
                    ...
                    <td><input name="url" type="text" value="${popConfig.url }" style="width: 250px; height: 20px;" placeholder="请输入链接地址" class="easyui-validatebox span2" ></td>
                </tr>
                <tr>
                	<td align="right" bgcolor="#f8f8f8">自动关闭时间（ 秒）</td>
                    <td>
                    	<input name="timing" value="${popConfig.timing }" style="width: 80px; height: 20px;" type="number" maxlength="10" data-options="required:true" value="0">
                    	<span style="color:red;">（默认0不自动关闭）</span>
                    </td>
                </tr>
                <tr>
                	<td align="right" bgcolor="#f8f8f8">备注</td>
                    <td><input name="note" type="text" value="${popConfig.note }" class="easyui-validatebox span2" ></td>
                </tr>
            </table>
        </form>
    </div>
