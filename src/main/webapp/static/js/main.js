...
$(function() {
     ...
     * 加载首页数据项
     */
    /*$.post(basePath+'/xcx/ba/getStatisData', {}, function (result) {
        if(result.success){
           $('#baCount').html(result.baCount);
           $('#dsrCount').html(result.dsrCount);
           $('#currentSales').html(result.currentSales);
           $('#sumSales').html(result.sumSales);
           $('#verifyUser').html(result.verifyUser);
        }
    },'json');*/
    
});
function addCloseTab(opts) {
    var t = $('#mainTabs');
...
function closeTab(opts) {
    $('#mainTabs').tabs('close', opts.title);
}
