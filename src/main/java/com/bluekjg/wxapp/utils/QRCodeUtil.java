...
public class QRCodeUtil extends LuminanceSource {
	 ...
	 private static final Logger logger = LoggerFactory.getLogger(QRCodeUtil.class);
	     ...
	     * @throws IOException
	     */
	    public static InputStream writeToStream(BitMatrix matrix,String ext) throws IOException {
	        BufferedImage image = toBufferedImage(matrix);
	     ...
	     * @throws Exception
	     */
	    public static InputStream generateQRCodeStream(String text, int width, int height, String format)
	            throws Exception {
	        Hashtable<EncodeHintType, Object> hints = new Hashtable<EncodeHintType, Object>();
	        hints.put(EncodeHintType.CHARACTER_SET, "utf-8");// 指定编码格式
	        hints.put(EncodeHintType.ERROR_CORRECTION, ErrorCorrectionLevel.L);// 指定纠错等级
	        hints.put(EncodeHintType.MARGIN, 1); // 白边大小，取值范围0~4
	        BitMatrix bitMatrix = new MultiFormatWriter().encode(text, BarcodeFormat.QR_CODE, width, height, hints);
	        InputStream is = writeToStream(bitMatrix, format);
	        return is;
	    }
	    ...
	    public static void main(String[] args) {
	        String text = "ORDER100000"; // 随机生成验证码
